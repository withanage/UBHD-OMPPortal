...
def bg_graph_model():
    graph = pgv.AGraph(layout='dot',  directed=True,
                       strict=False,  rankdir='LR')
...
def manage():
    ...
    grid = SQLFORM.smartgrid(table, args=request.args[
...
def hooks():
                ...
                for f in getattr(db[t], op):
                    if hasattr(f, '__call__'):
                        try:
                            ...
                            # Built in functions as delete_uploaded_files are
                            # not editable
                            if details['filename']:
                                details['url'] = URL(a='admin', c='default', f='edit', args=[request[
                                                     'application'], details['filename']], vars={'lineno': details['lineno']})
    ...
    for t in tables:
        ...
        ul_t = UL(_class='nav nav-list', _id="a_%s" %
                  t['slug'], _style='display:none')
        for op in t['method_hooks']:
            ul_t.append(LI(op['name']))
            ul_t.append(UL([LI(A(f['funcname'], _class="editor_filelink", _href=f[
                        'url']if 'url' in f else None, **{'_data-lineno': f['lineno'] - 1})) for f in op['functions']]))
        ul_main.append(ul_t)
    return ul_main
