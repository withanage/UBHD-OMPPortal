...
def book():
    ...
    pub_query = (db.publication_formats.submission_id == book_id) & (db.publication_format_settings.publication_format_id == db.publication_formats.publication_format_id) & (
        db.publication_format_settings.locale == locale) & (db.publication_format_settings.setting_value != myconf.take('omp.ignore_format'))

    publication_formats = db(pub_query).select(db.publication_format_settings.setting_name, db.publication_format_settings.setting_value,
    ...
    identification_codes = db(pub_query & (db.publication_format_settings.publication_format_id == db.identification_codes.publication_format_id)).select(
        db.identification_codes.value, db.publication_format_settings.setting_value)
    ...
    fullbook = db((db.submission_files.submission_id==book_id) & (db.submission_files.genre_id == 68)).select(db.submission_files.original_file_name, db.submission_files.submission_id, db.submission_files.genre_id,
                                        db.submission_files.file_id, db.submission_files.revision, db.submission_files.file_stage, db.submission_files.date_uploaded).first()
